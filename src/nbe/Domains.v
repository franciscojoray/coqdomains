...
Module RD : RecDom.
  ...
  Lemma ProjSet_inclusive T : admissible (@ProjSet T).
    move => c. unfold ProjSet. unfold admissible.
    intros C n.
  ...
  Definition kcpoCone (T:Tower kcpoBaseCatType) : Cone T.
    exists (sub_cpoType (@ProjSet_inclusive T))
  ...
  Lemma retract_total D E (f:D =-> E _BOT) (g:E =-> D _BOT) :
    kleisli f << g =-= eta -> PredomKleisli.total g.
  Proof.
...
Defined.
  ...
  Proof.
    move => c. simpl. unfold sum_fun. simpl. move => x. simpl.
...
(** Acá arranqué a definir cosas para biProd *)
  ...
  Definition biProd (F G : BiFunctor kcpoBaseCatType) : BiFunctor kcpoBaseCatType.
    exists (fun X Y     => (ob F X Y) * (ob G X Y))
...
rph F T1 T4 T3 T5)).
  ...
  Definition biConst (D:kcpoBaseCatType) : BiFunctor kcpoBaseCatType.
    exists (fun (X Y:kcpoBaseCatType) => D)
...
(*=DInf *)
  ...
  (*=ROLL *)
  Lemma foldT : PredomKleisli.total Fold. 
    (*CLEAR*)
  Proof.
    move => x. simpl.
  ...
  Qed.
  (*CLEARED*)
  Lemma unfoldT : PredomKleisli.total Unfold.  (*CLEAR*)
  Proof.
    move => x. simpl.
