...
Arguments  Category.morph [_ ].
(*CLEARED*)
Infix "<<" := Category.comp (at level 35) (*CLEAR*) : C_scope.

Arguments Category.comp [_ _ _ _].
(*CLEARED*)
...
Arguments sum_fun [_ C_scope _ _ S_scope _] : rename.
...
Lemma sum_fun_unique (C:sumCat) (X Y Z : C) (f:X =-> Z) (g:Y =-> Z) (h:X + Y =-> Z) :
    h << in1 =-= f -> h << in2 =-= g -> h =-= @sum_fun C _ _ _ f g.
...
Arguments  ev [_ C_scope _] : rename.
Arguments  exp [C_scope] : rename.
...
Add Parametric Morphism (C:expCat) (X Y Z:C) : (@exp_fun C X Y Z)
  with signature (@tset_eq (C (X * Y) Z)) ==> (@tset_eq (C X (@exp _ Y Z)))
  as exp_fun_eq_compat.
move => f f' e. apply: exp_unique.
by rewrite exp_com.
Qed.

Lemma exp_ev (Z:expCat) : forall (C D E:Z) (f:C =-> (@exp _ D E)) , exp_fun (ev << f >< Id ) =-= f.
intros Z C D E f.
symmetry.
by apply: exp_unique.
Qed.
...
Definition uncurry (Z:expCat) (D0 D1 D2 : Z) (f:D0 =-> (@exp _ D1  D2)) : D0 * D1 =-> D2 :=
      ev << <|f << pi1, pi2|>.
...
Lemma CU_id (C:expCat) (X Y Z:C) (f:X =-> (@exp _ Y  Z)) : exp_fun (uncurry f) =-= f.
...
Arguments  prodi_fun [_ _ C_scope _ S_scope ] : rename.

Lemma prodi_fun_pi (C:prodICat) I (Z:C) (A:I -> C) (f:forall i:I,Z =-> A i) (i:I):
                                                pi i << @prodi_fun _ _ _ _ f =-= f i.
...
Lemma prodi_fun_unique (C:prodICat) I (Z : C) (X:I -> C) (f:forall i, Z =-> X i) (h:Z =-> prodi X) :
    (forall i, pi i <<  h =-= f i) -> h =-= @prodi_fun _ _ _ _  f. 
...
Arguments functor_morph [C C' _ _] : rename .
...
{ conet :> C';
  ...
  conem_com : forall c0 c1 (m:C c0 c1), (@functor_morph _ _ F _ _  m) << conem c0 =-= conem c1 }.
...
{ coconet :> C';
  ...
  coconem_com : forall c0 c1 (m:C c0 c1), coconem c1 << (@functor_morph _  _ F _ _ m) =-= coconem c0 }.
